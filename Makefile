...
zipfile := ymir-aws-ebs-upload-$(version).zip
...
create-environment:
	aws elasticbeanstalk create-environment --application-name $(application-name) --environment-name $(environment-name) --description "Ymir production" --cname-prefix $(environment-name) --solution-stack-name $(aws-solution-stack)
